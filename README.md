...
```
data/testing/01.990145627_shuffled.pdf      # 개발/검증용 (39p)
data/validation/02.990367284_shuffled.pdf   # 최종 추론 대상 (44p)
data/testing_answers/*.pdf                  # package_01 원본 문서 4종 (GT 생성용)
```
...
- **[A] 추출** (`extractor.py`): PyMuPDF로 페이지별 텍스트·회전각·이미지 수·내부
  ...
  좌표 그대로 필터링합니다. 텍스트 200자 미만 페이지는 Vision 폴백 후보로 표시.
...
| 전체 (룰+LLM) | *API 키 설정 후 측정* | — | — | — | ≤2 (텍스트 1 + Vision 1) |

룰 단계의 오답 2건(p08 도면, p36 이미지 P&L)은 정확히 LLM 위임 대상으로 표시된
페이지들로, 룰의 실질 오분류(고신뢰 확정 후 오답)는 **0건**입니다. LLM 단계는 이
2페이지만 처리하면 되며, p36은 Vision 폴백 검증용 케이스입니다(이미지 위 얇은 텍스트
레이어). 전체 파이프라인 수치는 `ANTHROPIC_API_KEY` 설정 후 §1의 명령으로 재현할
수 있습니다.
...
전건은 [output/pkg01/error_analysis.md](output/pkg01/error_analysis.md) 참조
(페이지 번호·예측·정답·추정 원인·텍스트 스니펫 포함).

- **p08 (pred OTHER / gt TITLE_REPORT)**: plat map 플레이스홀더, 84자. 권원보고서
  구성물이지만 텍스트 단서가 없어 룰로 판별 불가 → Vision 폴백 대상.
- **p36 (pred OTHER / gt INCOME_DOC)**: P&L Statement. 이미지 위 얇은 텍스트
  레이어(250자)에 "Profit & Loss" 등 헤더 문구가 없고 숫자 나열뿐 → 텍스트 룰로
  판별 불가. 고용주명 "Veronica Salazar Realtor"가 노출되는데, "Realtor" 같은 일반
  단어를 INCOME 앵커로 쓰면 URLA 본문(같은 고용주명 등장)이 오분류되므로 의도적으로
  앵커에서 배제 → Vision 폴백 대상.

두 건 모두 "텍스트 신호 부재"가 원인이며 룰 사전 보강으로 풀 문제가 아니라 Vision
폴백으로 푸는 것이 옳은 케이스입니다(앵커를 느슨하게 만들면 §4의 함정처럼 정밀도가
무너짐).
...
측정치(rules-only 실행, Apple Silicon 로컬):

| 항목 | package_01 (39p) | package_02 (44p) |
|---|---|---|
| 전체 처리 시간 | ~0.2초 | ~0.2초 |
| 룰 단계 확정 비율 | 37/39 (94.9%) | 37/44 (84.1%) |
| LLM 위임 | 2페이지 | 7페이지 (Vision 후보 5 포함) |

LLM 단계 예상 비용(위임 페이지만 호출 시): 텍스트(Haiku 4.5, $1/$5 per MTok) 호출당
입력 ~1.5K 토큰 수준, Vision(Sonnet 4.6, $3/$15) 호출당 이미지 포함 입력 ~1.5–2K 토큰
수준으로, **패키지당 수 센트 이하**입니다. 순수 LLM 전체 분류 대비 호출 수가
39→2 (pkg01), 44→7 (pkg02)로 줄어드는 것이 하이브리드의 핵심 효과이며, 실제 호출
수·토큰·지연은 실행 시 `documents.json`의 `meta.cost`에 페이지별로 기록됩니다.
